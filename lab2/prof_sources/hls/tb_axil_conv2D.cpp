 ...
 */
void sw_convolution_2D(const int8_t *matrix_in, int8_t *matrix_out) {
    for(int l = 0; l < CONV_OFM_NUMBER; l ++)
        for (int i = 0; i < CONV_OUTPUT_HEIGHT; i++)
            for (int j = 0; j < CONV_OUTPUT_WIDTH; j++) {
                ...
                matrix_out[i * CONV_OUTPUT_WIDTH + j] = accum;
            }
}
...
int main() {
    ...
    for(int k = 0; k < CONV_OFM_NUMBER; k ++)
        for (int i = 0; i < CONV_OUTPUT_HEIGHT; i++) {
            for (int j = 0; j < CONV_OUTPUT_WIDTH; j++) {
                printf("%4d ", hw_image_out[k * CONV_OUTPUT_HEIGHT * CONV_OUTPUT_WIDTH + i * CONV_OUTPUT_WIDTH + j]);
            }
            printf("\n\r");
        }
    ...
    for(int k = 0; k < CONV_OFM_NUMBER; k ++)
        for (int i = 0; i < CONV_OUTPUT_HEIGHT; i++)
            for (int j = 0; j < CONV_OUTPUT_WIDTH; j++) {
                ...
                if (hw_image_out[ind] != sw_image_out[ind]) {
                    err_cnt++;
                    printf("%d,%d: %d != %d\n\r", i, j, hw_image_out[ind], sw_image_out[ind]);
                }
            }
