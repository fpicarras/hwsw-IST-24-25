...
static int8_t matA[N1*N2];
static int8_t matB[N2*N3];
static int8_t matC[N1*N3];
static int8_t matChw[N1*N3];

void axil_mat_prod1(int8_t m1[MAX_MEM_SIZE], int8_t m2[MAX_MEM_SIZE], int8_t m3[MAX_MEM_SIZE], int rowsA, int rowsB, int rowsC);
...
void print_mat(int8_t *x, int rows, int cols)
{
	int i;
	for (i=0; i<rows; i++) {
...
void SW_dot_product()
{
	...
	for (i=0; i<N1; i++) {
		for (j=0; j<N3; j++) {
			for (k=0; k<N2; k++) {
				int8_t mul = matA[i*N2+k] * matB[k*N3+j];
